...
public class DueniosDAO {
	...
	public boolean isDuenio(String documento) {
		Session s = HibernateUtil.getSessionFactory().openSession();
		s.beginTransaction();
		DuenioEntity duenioE = null;
		duenioE = (DuenioEntity) s.createQuery("FROM DuenioEntity de WHERE de.personaE.documento=?")
...
//	public void delete(Persona duenio, Unidad unidad) {
//		DuenioEntity duenioE = toEntity(duenio, unidad);
//		Session s = HibernateUtil.getSessionFactory().openSession();
//		s.beginTransaction();
//		s.delete(duenioE);
//		s.getTransaction().commit();
...
//	}
	...
	public void delete(int id) {
		Session s = HibernateUtil.getSessionFactory().openSession();
		s.beginTransaction();
		Query q = s.createQuery("DELETE DuenioEntity de WHERE de.unidadE.identificador = ?").setInteger(0, id);
		q.executeUpdate();
		s.getTransaction().commit();
		s.close();
	}
