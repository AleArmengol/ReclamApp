...
public class Persona {
	...
	/*
	 * Primero preguntar si es duenio, si es duenio no lo puedo borrar, si no es
	 * duenio y no es inquilino, lo borro directamente si es inquilino, le pido la
	 * lista de unidades de las que es inquilino. De cada unidada de esa lista
	 * pregunto si la lista que tiene de inquilinos es igual a 1, si es asi hago el
	 * update de la unidad a liberada una vez que hice todo esto elimino el
	 * inquilino
	 */
	public void delete() {
		// es duenio?
		if (!DueniosDAO.getInstance().isDuenio(this.documento)) {
			if (InquilinosDAO.getInstance().isInquilino(this.documento)) {
				...
				try {
					if (Controlador.getInstancia().inquilinosPorUnidad(unidadN.getEdificio().getCodigo(),
							unidadN.getPiso(), unidadN.getNumero()).size() == 1) {
